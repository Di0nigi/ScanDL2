...
def load_emtec():
    fixations_df = pd.read_csv(PATH_TO_EMTEC_FIX, sep='\t')
    stimuli_df = pd.read_csv(PATH_TO_EMTEC_STIM, sep='\t')
    return fixations_df, stimuli_df


def load_bsc():
    word_info_df = pd.read_excel(PATH_TO_BSC_WORD, sheet_name='word', engine='openpyxl')
    pos_info_df = pd.read_excel(PATH_TO_BSC_WORD, header=None, engine='openpyxl')
    eyemovement_df = pd.read_csv(PATH_TO_BSC_FIX, delimiter='\t')
...
def load_zuco(task: str = None):  # 'zuco11', 'zuco12'
    zuco_path = path_to_zuco
    if task.startswith('zuco1'):
        zuco_path = zuco_path + 'zuco/'
    elif task == 'zuco21':
        zuco_path = zuco_path + 'zuco2/'
    zuco_path = os.path.join(zuco_path, f'task{task[-1]}', 'Matlab_files')
...
def process_celer(
    ...
    data = {
        ...
        'sn_sp_fix_dur': list(),  # the fixation durations for the scanpath, padded with 0s for the sentence and to seq_len
    ...
    for sn_id_idx, sn_id in tqdm(enumerate(sn_list), total=len(sn_list)):  # for text/sentence ID
        ...
        for sub_id_idx, sub_id in enumerate(reader_list):
            ...
            # replace big fixation durations
            if max_fix_dur:
                sp_fix_dur[sp_fix_dur > max_fix_dur] = max_fix_dur
            ...
            sp_fix_dur = [0] + sp_fix_dur.tolist() + [0]
            ...
            sp_fix_dur.append(sep_token_sp_fix_dur)
    ...
    data['sn_sp_fix_dur'] = _collate_batch_helper(
        examples=data['sn_sp_fix_dur'],
        pad_token_id=0,
        max_length=args.seq_len,
    )
            ...
            train_dataset = Dataset2.from_dict(
                {
                    ...
                    'sn_sp_fix_dur': [instance[4] for instance in train_data],
                    'sn_repr_len': [instance[5] for instance in train_data],
                    'words_for_mapping': [instance[6] for instance in train_data],
                    'mask_sn_padding': [instance[7] for instance in train_data],
                    'mask_transformer_att': [instance[8] for instance in train_data],
                    'sn_ids': [instance[9] for instance in train_data],
                    'reader_ids': [instance[10] for instance in train_data],
                }
            )
            test_dataset = Dataset2.from_dict(
                {
                    ...
                    'sn_sp_fix_dur': [instance[4] for instance in test_data],
                    'sn_repr_len': [instance[5] for instance in test_data],
                    'words_for_mapping': [instance[6] for instance in test_data],
                    'mask_sn_padding': [instance[7] for instance in test_data],
                    'mask_transformer_att': [instance[8] for instance in test_data],
                    'sn_ids': [instance[9] for instance in test_data],
                    'reader_ids': [instance[10] for instance in test_data],
                }
            )
            ...
            val_dataset = Dataset2.from_dict(
                {
                    ...
                    'sn_sp_fix_dur': [instance[4] for instance in val_data],
                    'sn_repr_len': [instance[5] for instance in val_data],
                    'words_for_mapping': [instance[6] for instance in val_data],
                    'mask_sn_padding': [instance[7] for instance in val_data],
                    'mask_transformer_att': [instance[8] for instance in val_data],
                    'sn_ids': [instance[9] for instance in val_data],
                    'reader_ids': [instance[10] for instance in val_data],
                }
            )
            train_data_dict = datasets.DatasetDict()
...
def process_emtec(
    ...
    for name, group in tqdm(fixations_df.groupby(['subject_id', 'item_id'])):

        # disregard scanpaths with more than 200 fixations (49 scanpaths in 4473 overall scanpaths)
        if len(group) > 200:
            continue
        ...
        decoding_strategy = group['decoding_strategy'].values[0]

        sn_df = stimuli_df.loc[
            (stimuli_df['item_id'] == sn_id) &
            (stimuli_df['model'] == model) &
            (stimuli_df['decoding_strategy'] == decoding_strategy)
        ]

        # get the text 
        sn_str_list = sn_df['gen_seq_trunc'].values[0].split()
        sn_len = len(sn_str_list)
        sn_str = ' '.join(sn_str_list)
        sn_str = '[CLS] ' + sn_str + ' [SEP]'

        # fixation location
        sp_ordinal_pos = group['word_id'].astype(int).tolist()
        # add +1 because word ids start at 0 and we need CLS to be 0
        sp_ordinal_pos = [i + 1 for i in sp_ordinal_pos]

        # fixation durations 
        sp_fix_dur = group['fixation_duration'].astype(float).tolist()
        # replace big fixation durations
        if max_fix_dur:
            sp_fix_dur = [min(i, max_fix_dur) for i in sp_fix_dur]
        ...
        sn_word_ids = encoded_sn.word_ids()
        sp_word_ids = [0] + sp_ordinal_pos + [max(encoded_sn.word_ids())]
        sp_fix_dur = [0] + sp_fix_dur + [0]
        ...
        data['mask_transformer_att'].append(mask_transformer_att)
        data['sn_ids'].append(sn_id)
        data['reader_ids'].append(reader_id)
    ...
    if inference == 'cv':
        ...
        return NotImplementedError('Only cross-validation is supported for EMTEC data.')
...
def process_bsc(
    ...
    for name, group in tqdm(eyemovement_df.groupby(['id', 'sn'])):
        ...
        # fixation location (start from 1 in the corpus)
        sp_ordinal_pos = group['wn'].tolist()  

        # fixation duration 
        sp_fix_dur = group['dur'].tolist()
        if max_fix_dur:
            sp_fix_dur = [min(i, max_fix_dur) for i in sp_fix_dur]
    ...
    if inference == 'cv':
        ...
        return NotImplementedError('Only cross-validation is supported for BSC data.')