...
if __name__ == '__main__':
    ...
    parser.add_argument('--corpus', type=str, help='the eye-tracking corpus to use for training.', required=False,
                        default='celer', choices=['celer', 'zuco', 'emtec', 'bsc'])
    ...
    if args.inference == 'zuco':
        ...
        setting = args.data_split_criterion

    if args.corpus == 'emtec':
        model_file = os.path.join(SCANDL_MODULE_TRAIN_PATH_EMTEC, setting)
    elif args.corpus == 'bsc':
        model_file = os.path.join(SCANDL_MODULE_TRAIN_PATH_BSC, setting)
        args.config_name = 'bert-base-chinese'
    else:
        model_file = os.path.join(SCANDL_MODULE_TRAIN_PATH, setting)
