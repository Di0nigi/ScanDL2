...
from ScanDL2.scandl_fixdur.scandl_module.original_scandl.utils.fp16_util import (
    make_master_params,
...
from ScanDL2.scandl_fixdur.scandl_module.original_scandl.utils.nn import update_ema
from ScanDL2.scandl_fixdur.scandl_module.original_scandl.step_sample import LossAwareSampler, UniformSampler
...
class TrainLoop:
    ...
    def forward_backward(
        ...
        for i in range(0, batch['sn_sp_repr'].shape[0], self.microbatch):
            ...
            # the indices in t are the number of noising steps; how many times is noise added to each instance
            t, weights = self.schedule_sampler.sample(sn_sp_repr.shape[0], dist_util.dev())
            # print(micro_cond.keys())
            compute_losses = functools.partial(
                self.diffusion.training_losses,
                self.ddp_model,  # the transformer model
                t,  # the number of times to add noise to each instance
                sn_sp_repr,
                mask,
                sn_input_ids,
                indices_pos_enc,
    ...
    def optimize_normal(self):
        ...
        # log the gradient norm and the learning rate
        self._log_grad_norm()
        self._anneal_lr()
        self.opt.step()
        for rate, params in zip(self.ema_rate, self.ema_params):
