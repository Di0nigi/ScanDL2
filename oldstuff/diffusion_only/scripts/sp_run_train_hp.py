"""
Script to start the training process for hyper-parameter tuning.
"""
...
if __name__ == '__main__':
    ...
    if int(os.environ['LOCAL_RANK']) == 0:
        if not os.path.exists(DIFFUSION_ONLY_TRAIN_PATH):
            os.makedirs(DIFFUSION_ONLY_TRAIN_PATH)
    
    if int(os.environ['LOCAL_RANK']) == 0:
        if os.path.exists(DIFFUSION_ONLY_TRAIN_PATH):
            files = os.listdir(DIFFUSION_ONLY_TRAIN_PATH)
            trained = False
            for file in files:
                if file.endswith('.pt'):
                    trained = True
            if trained:
                print('\t\t####### \t fine-tuning already done on this config.')
                sys.exit()
    
    COMMANDLINE = f'TOKENIZERS_PARALLELISM=FALSE ' \
                  f'python -m scripts.sp_train_hp ' \
                  f'--checkpoint_path {DIFFUSION_ONLY_TRAIN_PATH} ' \
                  f'--vocab {args.vocab} ' \
                  f'--use_plm_init {args.use_plm_init} ' \
                  f'--lr {args.lr} ' \
                  f'--batch_size {args.bsz} ' \
                  f'--microbatch {args.bsz} ' \
    ...
    if int(os.environ['LOCAL_RANK']) == 0:
        with open(os.path.join(DIFFUSION_ONLY_TRAIN_PATH, 'saved_bash.sh'), 'w') as f:
